...
import { LinkedInJobSearchPage, LinkedInJobCard } from './pages/LinkedInJobSearchPage';
import { scrapeLinkedInDetail } from '../lib/scrape-linkedin';
...
// Route Google Sheets writes to the LinkedIn tab instead of the Xing tab.
// GOOGLE_SHEET_NAME_LN overrides the .env default; falls back to 'ln-raw-data'.
process.env.GOOGLE_SHEET_NAME = process.env.GOOGLE_SHEET_NAME_LN ?? 'ln-raw-data';

// Use 'LinkedIn' as the platform name for the Job Tracker push.
process.env.JOB_TRACKER_PLATFORM = 'LinkedIn';
...
/** Extract the numeric job ID from a LinkedIn job URL (stable across edits). */
function jobId(url: string): string {
  return url.match(/\/view\/(\d+)/)?.[1] ?? url;
}
...
function dedupeKey(card: LinkedInJobCard): string {
...
test.describe('LinkedIn multi-title job collection', () => {
  test.setTimeout(40 * 60_000);

  test('per-search scrape + profile match (Z.AI), write matches, keep top N', async ({ page }) => {
    const linkedin = new LinkedInJobSearchPage(page);
    ...
    await page.goto(linkedin.buildSearchUrl(titles[0], LOCATION, SINCE_PERIOD), { waitUntil: 'domcontentloaded' });
    await linkedin.acceptCookies();
    ...
    for (const [i, title] of titles.entries()) {
      await test.step(`Search "${title}" → scrape → score`, async () => {
        ...
        try {
          const cards = await linkedin.collectCardsForKeyword(title, LOCATION, SINCE_PERIOD);
          ...
          for (const card of relevant) {
            ...
            let job: JobRow;
            try {
              job = await scrapeLinkedInDetail(page, card);
            } catch (err) {
